...
from ..models.schemas import CheckResponse, ExcelDiffGridFocus, ExcelDiffGridResponse


class CheckServiceProcessingError(RuntimeError):
    """Raised when comparison processing fails after input validation."""
...
def _registered_file_infos(file_ids: Sequence[int]) -> List[Dict[str, Any]]:
    file_infos = []
    for file_id in file_ids:
        file_row = get_file_by_id(file_id)
        if not file_row:
            raise FileNotFoundError(f"등록되지 않은 파일입니다. (id={file_id})")
        if not _path_is_accessible(file_row["path"]):
            raise FileNotFoundError(f"파일이 삭제되었거나 경로가 변경되었습니다: {file_row['path']}")
        file_infos.append(
            {
                "id": file_row["id"],
                "path": file_row["path"],
...
def run_consistency_check_for_file_ids(file_ids: Sequence[int]) -> CheckResponse:
    """Run document comparison as an application use case, not a FastAPI handler."""

    if len(file_ids) < 2:
        raise ValueError("정합성 검사는 최소 2개 파일을 선택해야 합니다.")

    comparison_mode = "version_history"
    file_infos = _registered_file_infos(file_ids)
    ...
    try:
        ...
        raise CheckServiceProcessingError(f"정합성 검사 중 오류가 발생했습니다: {exc}") from exc

    result["metadata"] = _merge_metadata(result.get("metadata"), source_metadata)
    response = CheckResponse(**result)
    save_cached_comparison_result(cache_key, list(file_ids), comparison_mode, response.model_dump())
    return response


def build_excel_diff_grid_for_file_ids(
    file_ids: Sequence[int],
    focuses: Sequence[ExcelDiffGridFocus],
    ...
    for file_info in file_infos:
        if file_info["file_type"] != "Excel":
            raise ValueError("표로 보기는 Excel 파일만 지원합니다.")

    try:
        result = build_excel_diff_grid(file_infos, focuses)
    except ValueError:
        raise
    except Exception as exc:
        raise CheckServiceProcessingError(f"Excel 표를 만드는 중 오류가 발생했습니다: {exc}") from exc
